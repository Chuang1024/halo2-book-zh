...
我们将 $\sec$ 作为安全参数，除非特别说明，所有算法和对手都是概率（交互式）图灵机，其运行时间在安全参数 $\sec$ 下是多项式时间的。我们用 $\negl$ 表示在 $\sec$ 下可忽略的函数。

### 密码群

我们用 $\group$ 表示一个素数阶 $p$ 的循环群。群的单位元记为 $\zero$。我们将群元素的标量称为大小为 $p$ 的标量域 $\field$ 中的元素。群元素用大写字母表示，标量用小写字母或希腊字母表示。标量或群元素的向量用粗体表示，例如 $\mathbf{a} \in \field^n$ 和 $\mathbf{G} \in \group^n$。群操作以加法表示，群元素 $G$ 与标量 $a$ 的乘法记为 $[a] G$。

我们经常使用符号 $\langle \mathbf{a}, \mathbf{b} \rangle$ 来描述两个相同长度的标量向量 $\mathbf{a}, \mathbf{b} \in \field^n$ 的内积。我们也用这个符号表示群元素的线性组合，例如 $\langle \mathbf{a}, \mathbf{G} \rangle$，其中 $\mathbf{a} \in \field^n, \mathbf{G} \in \group^n$，实际计算中通过多标量乘法实现。

我们用 $\mathbf{0}^n$ 表示长度为 $n$ 且仅包含 $\field$ 中零元素的向量。
...
$$
\begin{array}{ll}
&\underline{\bold{游戏} \, \mathsf{G}^\dlrel_{\group,n}(\a, \sec):} \\
&\mathbf{G} \gets \group^n_\sec \\
&\mathbf{a} \gets \a(\mathbf{G}) \\
&\textnormal{返回} \, \left( \langle \mathbf{a}, \mathbf{G} \rangle = \zero \land \mathbf{a} \neq \mathbf{0}^n \right)
\end{array}
$$

给定一个长度为 $n$ 的群元素向量 $\mathbf{G} \in \group^n$，_离散对数关系问题_ 要求找到 $\mathbf{g} \in \field^n$ 使得 $\mathbf{g} \neq \mathbf{0}^n$ 且 $\innerprod{\mathbf{g}}{\mathbf{G}} = \zero$，我们称之为 _非平凡_ 的离散对数关系。该问题的困难性紧密依赖于群中离散对数问题的困难性，如 [[JT20]](https://eprint.iacr.org/2020/1213) 中的引理 3 所示。形式上，我们使用上述定义的游戏 $\dlgame$ 来捕捉这个问题。

### 交互式证明

_交互式证明_ 是一个三元组算法 $\ip = (\setup, \prover, \verifier)$。算法 $\setup(1^\sec)$ 生成一些 _公共参数_，通常记为 $\pp$。证明者 $\prover$ 和验证者 $\verifier$ 是交互式机器（可以访问 $\pp$），我们通过 $\langle \prover(x), \verifier(y) \rangle$ 表示它们在输入 $x, y$ 上执行的双边协议。该协议的输出是它们的交互 _记录_，包含所有在 $\prover$ 和 $\verifier$ 之间发送的消息。协议结束时，验证者输出一个决策位。

### 零知识知识论证

知识证明是交互式证明，其中证明者旨在让验证者相信他们知道一个见证 $w$，使得 $(x, w) \in \relation$，其中 $x$ 是陈述，$\relation$ 是多项式时间可判定的关系。我们将使用 _论证_ 知识，假设证明者是计算受限的。

我们将通过四个安全概念来分析知识论证。

* **完备性：​** 如果证明者拥有有效的见证，他们是否 _总是_ 能说服验证者？理解这一属性是有用的，因为它可能对其他安全概念有影响。
* **可靠性：​** 作弊的证明者是否能错误地说服验证者一个实际上不正确的陈述？我们将作弊证明者错误说服验证者的概率称为 _可靠性错误_。
* **知识可靠性：​** 当验证者确信陈述正确时，证明者是否实际拥有（“知道”）有效的见证？我们将作弊证明者错误说服验证者这一知识的概率称为 _知识错误_。
* **零知识性：​** 验证者除了从陈述的正确性和证明者拥有有效见证的知识中推断出的信息外，是否还能学到其他信息？

首先，我们将讨论完备性的简单定义。

> **完美完备性。​** 一个交互式论证 $(\setup, \prover, \verifier)$ 具有 _完美完备性_，如果对于所有多项式时间可判定的关系 $\relation$ 和所有非均匀多项式时间的对手 $\a$，

$$
Pr \left[
    (x, w) \notin \relation \lor
    \langle \prover(\pp, x, w), \verifier(\pp, x) \rangle \, \textnormal{接受}
    \, \middle| \,
    \begin{array}{ll}
    &\pp \gets \setup(1^\sec) \\
    &(x, w) \gets \a(\pp)
...
尽管我们的协议被描述为交互式论证，但在实践中，它通过使用 Fiat-Shamir 变换被实现为 _非交互式论证_，这使我们的分析变得复杂。

> **公共硬币。​** 我们说一个交互式论证是 _公共硬币_ 的，当验证者发送的所有消息都是通过新鲜随机性采样的。

> **Fiat-Shamir 变换。​** 在这种变换中，一个交互式的、公共硬币的论证可以通过将验证者算法替换为产生足够随机输出的加密强哈希函数，在 _随机预言机模型_ 中变为 _非交互式_。

这种变换意味着在具体协议中，作弊的证明者可以通过分叉记录并发送新消息给验证者来轻松“回放”验证者。研究在应用这种变换后我们构造的具体安全性是重要的。幸运的是，我们能够遵循 Ghoshal 和 Tessaro（[[GT20](https://eprint.iacr.org/2020/1351)]）的分析框架，该框架已应用于与我们构造类似的协议。

我们将通过 _状态恢复可靠性_ 的概念来研究我们的协议。在这种模型中，（作弊的）证明者被允许将验证者回放到任何先前的状态。如果证明者能够产生一个被接受的记录，则他们获胜。

> **状态恢复可靠性。​** 设 $\ip$ 是一个具有 $r = r(\sec)$ 个验证者挑战的交互式论证，且第 $i$ 个挑战从 $\ch_i$ 中采样。状态恢复证明者 $\prover$ 的优势度量
$$
\adv^\srs_\ip(\prover, \sec) = \textnormal{Pr} \left[ \srs^\ip_\prover(\sec) \right]
$$
> 定义如下游戏。
...
\begin{array}{ll}
\begin{array}{ll}
&\underline{\bold{游戏} \, \srs_\ip^\prover(\sec):} \\
&\textnormal{win} \gets \tt{false}; \\
&\tr \gets \epsilon \\
&\pp \gets \ip.\setup(1^\sec) \\
&(x, \textsf{st}_\prover) \gets \prover_\sec(\pp) \\
&\textnormal{运行} \, \prover^{\oracle_\srs}_\sec(\textsf{st}_\prover) \\
&\textnormal{返回 win}
\end{array} &
\begin{array}{ll}
&\underline{\bold{预言机} \, \oracle_\srs(\tau = (a_1, c_1, ..., a_{i - 1}, c_{i - 1}), a_i):} \\
& \textnormal{如果} \, \tau \in \tr \, \textnormal{那么} \\
& \, \, \textnormal{如果} \, i \leq r \, \textnormal{那么} \\
& \, \, \, \, c_i \gets \ch_i; \tr \gets \tr || (\tau, a_i, c_i); \textnormal{返回} \, c_i \\
& \, \, \textnormal{否则如果} \, i = r + 1 \, \textnormal{那么} \\
& \, \, \, \, d \gets \ip.\verifier (\pp, x, (\tau, a_i)); \tr \gets (\tau, a_i) \\
& \, \, \, \, \textnormal{如果} \, d = 1 \, \textnormal{那么 win} \gets \tt{true} \\
& \, \, \, \, \textnormal{返回} \, d \\
&\textnormal{返回} \bottom
\end{array}
\end{array}
...
如 [[GT20](https://eprint.iacr.org/2020/1351)]（定理 1）所示，状态恢复可靠性与应用 Fiat-Shamir 变换后的可靠性紧密相关。

## 知识可靠性

我们将展示我们的协议满足一种强化的知识可靠性概念，称为 _见证扩展模拟_。非正式地，这个概念表明，对于任何成功的证明者算法，存在一个高效的 _模拟器_，可以通过回放它并提供新的随机性来从中提取见证。

然而，我们必须稍微调整我们对见证扩展模拟的定义，以考虑到我们的证明者是状态恢复证明者并且可以回放验证者。此外，为了避免在见证提取过程中回放状态恢复证明者，我们在代数群模型中研究我们的协议。

> **代数群模型（AGM）。​** 如果对手 $\alg{\prover}$ 在输出群元素 $X$ 时也输出一个 _表示_ $\mathbf{x} \in \field^n$ 使得 $\langle \mathbf{x}, \mathbf{G} \rangle = X$，其中 $\mathbf{G} \in \group^n$ 是 $\alg{\prover}$ 到目前为止看到的群元素向量，则称其为 _代数的_。
> 在符号上，我们写 $\rep{X}$ 来描述增强了这种表示的群元素 $X$。我们还写 $\repv{X}{G}{i}$ 来标识 $X$ 的表示中对应于 $\mathbf{G}_i$ 的分量。换句话说，
$$
X = \sum\limits_{i=0}^{n - 1} \left[ \repv{X}{G}{i} \right] \mathbf{G}_i
$$

代数群模型允许我们对某些协议执行所谓的“在线”提取：提取器可以从单个（被接受的）记录中的表示本身获取见证。

> **状态恢复见证扩展模拟** 设 $\ip$ 是一个针对关系 $\relation$ 的具有 $r = r(\sec)$ 个挑战的交互式论证。我们为所有非均匀代数证明者 $\alg{\prover}$、提取器 $\extractor$ 和计算上无界的区分器 $\distinguisher$ 定义优势度量
$$
\adv^\srwee_{\ip, \relation}(\alg{\prover}, \distinguisher, \extractor, \sec) = \textnormal{Pr} \left[ \weereal^{\prover,\distinguisher}_{\ip,\relation}(\sec) \right] - \textnormal{Pr} \left[ \weeideal^{\extractor,\prover,\distinguisher}_{\ip,\relation}(\sec) \right]
$$
> 定义如下游戏。
...
\begin{array}{ll}
\begin{array}{ll}
&\underline{\bold{游戏} \, \weereal_{\ip,\relation}^{\alg{\prover},\distinguisher}(\sec):} \\
&\tr \gets \epsilon \\
&\pp \gets \ip.\setup(1^\sec) \\
&(x, \state{\prover}) \gets \alg{\prover}(\pp) \\
&\textnormal{运行} \, \alg{\prover}^{\oracle_\real}(\state{\prover}) \\
&b \gets \distinguisher(\tr) \\
&\textnormal{返回} \, b = 1 \\
&\underline{\bold{游戏} \, \weeideal_{\ip,\relation}^{\extractor,\alg{\prover},\distinguisher}(\sec):} \\
&\tr \gets \epsilon \\
&\pp \gets \ip.\setup(1^\sec) \\
&(x, \state{\prover}) \gets \alg{\prover}(\pp) \\
&\state{\extractor} \gets (1^\sec, \pp, x) \\
&\textnormal{运行} \, \alg{\prover}^{\oracle_\ideal}(\state{\prover}) \\
&w \gets \extractor(\state{\extractor}, \bottom) \\
&b \gets \distinguisher(\tr) \\
&\textnormal{返回} \, (b = 1) \\
&\, \, \land (\textnormal{Acc}(\tr) \implies (x, w) \in \relation) \\
\end{array} &
\begin{array}{ll}
&\underline{\bold{预言机} \, \oracle_\real(\tau = (a_1, c_1, ..., a_{i - 1}, c_{i - 1}), a_i):} \\
& \textnormal{如果} \, \tau \in \tr \, \textnormal{那么} \\
& \, \, \textnormal{如果} \, i \leq r \, \textnormal{那么} \\
& \, \, \, \, c_i \gets \ch_i; \tr \gets \tr || (\tau, a_i, c_i); \textnormal{返回} \, c_i \\
& \, \, \textnormal{否则如果} \, i = r + 1 \, \textnormal{那么} \\
...
\\
\\
&\underline{\bold{预言机} \, \oracle_\ideal(\tau, a):} \\
& \textnormal{如果} \, \tau \in \tr \, \textnormal{那么} \\
& \, \, (r, \state{\extractor}) \gets \extractor(\state{\extractor}, \left[(\tau, a)\right]) \\
& \, \, \tr \gets \tr || (\tau, a, r) \\
& \, \, \textnormal{返回} \, r \\
...
如果验证者除了能够从有效见证$w$的存在性中获知信息外，无法从交互过程中学习到任何额外信息，我们称这种知识论证是**零知识的**。更形式化地定义如下：

> **完美特殊诚实验证者零知识性（PSHVZK）。** 一个公开掷币的交互式论证$(\setup, \prover, \verifier)$具有**完美特殊诚实验证者零知识性**，如果对于所有多项式时间可判定的关系$\relation$，对于所有$(x, w) \in \relation$，以及所有非均匀多项式时间敌手$\a_1, \a_2$，存在一个概率多项式时间模拟器$\sim$，使得以下等式成立：
$$
\begin{array}{rl}
&Pr \left[ \a_1(\sigma, x, \tr) = 1 \, \middle| \,
\begin{array}{ll}
...
在这种（常见的）零知识性定义中，验证者被期望“诚实地”行动，仅根据其内部随机性发送挑战；他们不能基于证明者的消息自适应地响应。我们使用了一种强化形式的定义，强制模拟器输出一个与验证者算法发送给证明者的挑战相同的交互记录。

## 协议

设$\omega \in \field$是一个$n = 2^k$次本原单位根，形成域$D = (\omega^0, \omega^1, ..., \omega^{n - 1})$，$t(X) = X^n - 1$是该域上的消失多项式。设$n_g, n_a, n_e$为正整数，满足$n_a, n_e \lt n$且$n_g \geq 4$。我们提出一个交互式论证$\halo = (\setup, \prover, \verifier)$，用于以下关系：
$$
\relation = \left\{
\begin{array}{ll}
&\left(
...
\right\}
$$
其中$a_0, a_1, ..., a_{n_a - 1}$是（多元）多项式，在$X$上的次数为$n - 1$，而$g$在任意变量$X, C_0, C_1, ...$上的次数最多为$n_g(n - 1)$。

$\setup(\sec)$返回$\pp = (\group, \field, \mathbf{G} \in \group^n, U, W \in \group)$。

对于所有$i \in [0, n_a)$：
• 设$\mathbf{p_i}$为整数$j$（模$n$）的穷举集合，使得$a_i(\omega^j X, \cdots)$作为一项出现在$g(X, \cdots)$中。
• 设$\mathbf{q}$为一个包含$\mathbf{p_i}$和集合$\mathbf{q_0} = \{0\}$的不同整数集合的列表。
• 当$\mathbf{q}_j = \mathbf{p_i}$时，设$\sigma(i) = \mathbf{q}_j$。

设$n_q \leq n_a$表示$\mathbf{q}$的大小，且不失一般性，设$n_e$为每个$\mathbf{p_i}$的大小。

在以下协议中，我们默认每个多项式$a_i(X, \cdots)$的定义使得证明者新采样了$n_e + 1$个盲因子，并且每个盲因子都作为$a_i(X, \cdots)$在域$D$上的一个求值出现。在所有后续内容中，验证者的挑战不能为零或域$D$中的元素，并且对特定挑战还有一些额外的限制。

1. $\prover$和$\verifier$进行以下$n_a$轮交互，其中在第$j$轮（从$0$开始）：
    • $\prover$设置$a'_j(X) = a_j(X, c_0, c_1, ..., c_{j - 1}, a_0(X, \cdots), ..., a_{j - 1}(X, \cdots, c_{j - 1}))$。
    • $\prover$发送一个隐藏承诺$A_j = \innerprod{\mathbf{a'}}{\mathbf{G}} + [\cdot] W$，其中$\mathbf{a'}$是单变量多项式$a'_j(X)$的系数，$\cdot$是为了简化说明而省略的随机独立采样的盲因子。（这种省略符号在整个协议描述中用于简化说明。）
    • $\verifier$回应一个挑战$c_j$。
2. $\prover$设置$g'(X) = g(X, c_0, c_1, ..., c_{n_a - 1}, \cdots)$。
3. $\prover$发送一个承诺$R = \innerprod{\mathbf{r}}{\mathbf{G}} + [\cdot] W$，其中$\mathbf{r} \in \field^n$是随机采样的单变量多项式$r(X)$的系数，次数为$n - 1$。
4. $\prover$计算单变量多项式$h(X) = \frac{g'(X)}{t(X)}$，次数为$n_g(n - 1) - n$。
5. $\prover$计算最多$n - 1$次的多项式$h_0(X), h_1(X), ..., h_{n_g - 2}(X)$，使得$h(X) = \sum\limits_{i=0}^{n_g - 2} X^{ni} h_i(X)$。
6. $\prover$发送承诺$H_i = \innerprod{\mathbf{h_i}}{\mathbf{G}} + [\cdot] W$，其中$\mathbf{h_i}$表示$h_i(X)$的系数向量。
7. $\verifier$回应挑战$x$并计算$H' = \sum\limits_{i=0}^{n_g - 2} [x^{ni}] H_i$。
8. $\prover$设置$h'(X) = \sum\limits_{i=0}^{n_g - 2} x^{ni} h_i(X)$。
9. $\prover$发送$r = r(x)$，并且对于所有$i \in [0, n_a)$发送$\mathbf{a_i}$，使得$(\mathbf{a_i})_j = a'_i(\omega^{(\mathbf{p_i})_j} x)$，对于所有$j \in [0, n_e - 1]$。
10. 对于所有$i \in [0, n_a)$，$\prover$和$\verifier$设置$s_i(X)$为最低次数的单变量多项式，满足$s_i(\omega^{(\mathbf{p_i})_j} x) = (\mathbf{a_i})_j$，对于所有$j \in [0, n_e - 1)$。
11. $\verifier$回应挑战$x_1, x_2$并初始化$Q_0, Q_1, ..., Q_{n_q - 1} = \zero$。
    • 从$i=0$开始到$n_a - 1$结束，$\verifier$设置$Q_{\sigma(i)} := [x_1] Q_{\sigma(i)} + A_i$。
    • $\verifier$最后设置$Q_0 := [x_1^2] Q_0 + [x_1] H' + R$。
12. $\prover$初始化$q_0(X), q_1(X), ..., q_{n_q - 1}(X) = 0$。
    • 从$i=0$开始到$n_a - 1$结束，$\prover$设置$q_{\sigma(i)} := x_1 q_{\sigma(i)} + a'(X)$。
    • $\prover$最后设置$q_0(X) := x_1^2 q_0(X) + x_1 h'(X) + r(X)$。
13. $\prover$和$\verifier$初始化$r_0(X), r_1(X), ..., r_{n_q - 1}(X) = 0$。
    • 从$i = 0$开始到$n_a - 1$结束，$\prover$和$\verifier$设置$r_{\sigma(i)}(X) := x_1 r_{\sigma(i)}(X) + s_i(X)$。
    • 最后，$\prover$和$\verifier$设置$r_0 := x_1^2 r_0 + x_1 h + r$，其中$h$由$\verifier$通过$\frac{g'(x)}{t(x)}$计算，使用$\prover$提供的值$r, \mathbf{a}$。
14. $\prover$发送$Q' = \innerprod{\mathbf{q'}}{\mathbf{G}} + [\cdot] W$，其中$\mathbf{q'}$定义多项式的系数：
  $$q'(X) = \sum\limits_{i=0}^{n_q - 1}
  x_2^i
    \left(
    \frac
    ...
    \right)
$$
15. $\verifier$回应挑战$x_3$。
16. $\prover$发送$\mathbf{u} \in \field^{n_q}$，使得$\mathbf{u}_i = q_i(x_3)$，对于所有$i \in [0, n_q)$。
17. $\verifier$回应挑战$x_4$。
18. $\verifier$设置$P = Q' + x_4 \sum\limits_{i=0}^{n_q - 1} [x_4^i] Q_i$和$v = $
$$
\sum\limits_{i=0}^{n_q - 1}
\left(
x_2^i
...
20. $\prover$采样一个在$x_3$处有根的随机多项式$s(X)$，次数为$n - 1$，并发送一个承诺$S = \innerprod{\mathbf{s}}{\mathbf{G}} + [\cdot] W$，其中$\mathbf{s}$定义$s(X)$的系数。
21. $\verifier$回应挑战$\xi, z$。
22. $\verifier$设置$P' = P - [v] \mathbf{G}_0 + [\xi] S$。
23. $\prover$设置$p'(X) = p(X) - p(x_3) + \xi s(X)$（其中$p(x_3)$应与验证者计算的值$v$对应）。
24. 初始化$\mathbf{p'}$为$p'(X)$的系数，$\mathbf{G'} = \mathbf{G}$和$\mathbf{b} = (x_3^0, x_3^1, ..., x_3^{n - 1})$。$\prover$和$\verifier$将在以下$k$轮中交互，其中第$j$轮从$j=0$开始到$j=k-1$结束：
  • $\prover$发送$L_j = \innerprod{\mathbf{p'}_\hi}{\mathbf{G'}_\lo} + [z \innerprod{\mathbf{p'}_\hi}{\mathbf{b}_\lo}] U + [\cdot] W$和$R_j = \innerprod{\mathbf{p'}_\lo}{\mathbf{G'}_\hi} + [z \innerprod{\mathbf{p'}_\lo}{\mathbf{b}_\hi}] U + [\cdot] W$。
  • $\verifier$回应挑战$u_j$，选择使得$1 + u_{k-1-j} x_3^{2^j}$非零。
  • $\prover$和$\verifier$设置$\mathbf{G'} := \mathbf{G'}_\lo + u_j \mathbf{G'}_\hi$和$\mathbf{b} := \mathbf{b}_\lo + u_j \mathbf{b}_\hi$。
  • $\prover$设置$\mathbf{p'} := \mathbf{p'}_\lo + u_j^{-1} \mathbf{p'}_\hi$。
25. $\prover$发送$c = \mathbf{p'}_0$和合成盲因子$f$，由省略的盲因子计算得出。
26. $\verifier$仅在$\sum_{j=0}^{k - 1} [u_j^{-1}] L_j + P' + \sum_{j=0}^{k - 1} [u_j] R_j = [c] \mathbf{G'}_0 + [c \mathbf{b}_0 z] U + [f] W$时接受。

### 零知识性和完备性

我们声称该协议是**完美完备的**。这可以通过检查协议来验证；给定一个有效的见证$a_i(X, \cdots) \forall i$，证明者以概率$1$成功说服验证者。

我们声称该协议是**完美特殊诚实验证者零知识的**。我们通过展示存在一个模拟器$\sim$，可以产生一个与具有相同公开硬币的验证者与有效证明者交互的接受记录，且分布相同。模拟器将像一个诚实的证明者一样行动，但有以下例外：

1. 在协议的第$1$步中，$\sim$选择随机的$n - 1$次多项式（在$X$中）$a_i(X, \cdots) \forall i$。
2. 在协议的第$5$步中，$\sim$选择随机的$n - 1$次多项式$h_0(X), h_1(X), ..., h_{n_g - 2}(X)$。
3. 在协议的第$14$步中，$\sim$选择一个随机的$n - 1$次多项式$q'(X)$。
4. 在协议的第$20$步中，$\sim$利用其对验证者选择$\xi$的预知，产生一个$n - 1$次多项式$s(X)$，仅满足$p(X) - v + \xi s(X)$在$x_3$处有根。

首先，让我们考虑为什么这个模拟器总能产生一个**接受**的记录。$\sim$缺乏有效的见证，只是在诚实证明者需要有效见证时承诺于随机多项式。验证者对记录中的标量值没有条件。$\sim$只需保证协议的第$26$步中的检查成功。它通过利用对挑战$\xi$的知识产生一个多项式，干扰$p'(X)$以确保其在$x_3$处有根。因此，由于完美完备性，记录总是被接受。

为了理解为什么$\sim$产生的记录与诚实证明者的记录分布相同，我们将查看记录的每一部分并比较分布。首先，注意$\sim$（与诚实证明者一样）为记录中的每个群元素使用了一个全新的随机盲因子，因此我们只需要考虑记录中的**标量**。$\sim$与证明者的行为相同，除了上述情况，因此我们将分析每种情况：

1. $\sim$和诚实证明者揭示了每个多项式$a_i(X, \cdots)$的$n_e$个开点，以及在第$16$步中每个$a_i(X, \cdots)$的最多一个额外开点。然而，诚实证明者用$n_e + 1$个在域$D$上的随机求值对其多项式$a_i(X, \cdots)$（在$X$中）进行了盲化。因此，在挑战$x$（协议禁止其为$0$或域$D$中的元素）处$a_i(X, \cdots)$的开点在$\sim$和诚实证明者之间分布相同。
2. $\sim$和诚实证明者都没有揭示$h(x)$，因为它由验证者计算。然而，诚实证明者可能揭示$h'(x_3)$——它与$h(X)$有非平凡的关系——如果不是因为诚实证明者在第$3$步中还承诺了一个随机的$n - 1$次多项式$r(X)$，产生了一个承诺$R$，并确保在第$12$步中当证明者设置$q_0(X) := x_1^2 q_0(X) + x_1 h'(X) + r(X)$时，$q_0(x_3)$的分布是均匀随机的。因此，$h'(x_3)$既没有被诚实证明者揭示，也没有被$\sim$揭示。
3. $q'(x_3)$的期望值由验证者计算（在第$18$步中），因此模拟器对$q'(X)$的实际选择无关紧要。
4. $p(X) - v + \xi s(X)$被条件化为在$x_3$处有根，但除此之外对$s(X)$没有其他条件，因此无论$s(X)$是否在$x_3$处有根，$n - 1$次多项式$p(X) - v + \xi s(X)$的分布都是均匀随机的。因此，在第$25$步中产生的$c$的分布在$\sim$和诚实证明者之间是相同的。在第$25$步中揭示的合成盲因子$f$也是证明者其他盲因子的一个平凡函数，因此在$\sim$和诚实证明者之间分布相同。

笔记

1. 在我们协议的早期版本中，证明者会在多点打开论证中打开每个单独的承诺$H_0, H_1, ...$，验证者会确认这些开点的线性组合（以$x^n$的幂）与$h(x)$的期望值一致。这样做是因为在递归证明中更高效。然而，我们不清楚这些承诺$H_0, H_1, ...$的开点的期望分布是什么，因此证明该论证是零知识的很困难。相反，我们改变了论证，使得**验证者**计算承诺的线性组合，并在$x$处打开该线性组合。这避免了泄露$h_i(x)$。
2. 如前所述，在第$3$步中，证明者承诺了一个随机多项式，以确保在多开论证中不泄露$h'(x_3)$。这是因为$h'(x_3)$的分布不明确。
3. 从技术上讲，我们也可以通过使用模拟器来证明零知识性，该模拟器利用其对挑战$x$的预知来承诺一个$h(X)$，使得其在$x$处与期望值一致。这将消除协议中随机多项式$s(X)$的需要。但这可能会使协议其余部分的零知识性分析变得复杂，因此我们没有采用这种方法。
4. 在第$23$步中，多项式完全随机化后，**群元素的盲因子**在技术上不再必要。然而，在实践中，我们更简单的方法是确保协议中的每个群元素都被随机盲化，以避免涉及无穷远点的边缘情况。
5. 至关重要的是，验证者不能挑战证明者在域$D$中的点打开多项式，否则诚实证明者的记录将被迫包含可能是证明者见证的部分。因此，我们将挑战空间限制为域中除$D$外的所有元素，并且为了简单起见，我们还禁止挑战$0$。

## 见证扩展模拟

设$\protocol = \protocol[\group]$是上述为关系$\relation$和某个群$\group$（其标量域为$\field$）描述的交互式论证。我们总是可以构造一个提取器$\extractor$，使得对于任何最多向其预言机进行$q$次查询的非均匀代数证明者$\alg{\prover}$，存在一个非均匀敌手$\dlreladv$，满足对于任何计算上无界的区分器$\distinguisher$：
$$
\adv^\srwee_{\protocol, \relation}(\alg{\prover}, \distinguisher, \extractor, \sec) \leq q\epsilon + \adv^\dlrel_{\group,n+2}(\dlreladv, \sec)
$$
其中$\epsilon \leq \frac{n_g \cdot (n - 1)}{|\ch|}$。

**证明。** 我们将通过调用[[GT20]](https://eprint.iacr.org/2020/1351)的定理1来证明这一点。首先，我们注意到所有轮的挑战空间是相同的，即$\forall i \ \ch = \ch_i$。定理1要求我们定义：

• 对于所有部分记录$\tr' = (\pp, x, [a_0], c_0, \ldots, [a_i])$，定义$\badch(\tr') \in \ch$，使得$|\badch(\tr')| / |\ch| \leq \epsilon$。
• 一个提取器函数$e$，它接受一个接受的扩展记录$\tr$，并返回一个有效见证或失败。
• 一个函数$\pfail(\protocol, \alg{\prover}, e, \relation)$，返回一个概率。

我们说一个接受的扩展记录$\tr$包含“坏挑战”，当且仅当存在一个部分扩展记录$\tr'$，一个挑战$c_i \in \badch(\tr')$，以及一些证明者消息和挑战的序列$([a_{i+1}], c_{i+1}, \ldots [a_j])$，使得$\tr = \tr' \,||\, (c_i, [a_{i+1}], c_{i+1}, \ldots [a_j])$。

定理1要求$e$在给定一个不包含“坏挑战”的接受扩展记录$\tr$时，返回该记录的有效见证，除非概率不超过$\pfail(\protocol, \alg{\prover}, e, \relation)$。

我们的策略如下：我们将定义$e$，建立$\pfail$相对于玩$\dlrel_{\group,n+2}$游戏的敌手$\dlreladv$的上界，将这些代入定理1，然后遍历协议以确定$\badch(\tr')$大小的上界。敌手$\dlreladv$玩$\dlrel_{\group,n+2}$游戏如下：给定输入$U, W \in \mathbb{G}, \mathbf{G} \in \mathbb{G}^n$，敌手$\dlreladv$使用$\dlrel_{\group,n+2}$游戏的输入作为公共参数，模拟$\srwee_{\protocol, \relation}$游戏给$\alg{\prover}$。如果$\alg{\prover}$成功产生一个接受的扩展记录$\tr$，$\dlreladv$调用一个函数$h$处理$\tr$并返回其输出。我们将以这样的方式定义$h$，使得对于一个不包含“坏挑战”的接受扩展记录$\tr$，当$h(\tr)$**不**返回一个非平凡的离散对数关系时，$e(\tr)$**总是**返回一个有效见证。这意味着概率$\pfail(\protocol, \alg{\prover}, e, \relation)$不超过$\adv^\dlrel_{\group,n+2}(\dlreladv, \sec)$，从而证明了我们的主张。

#### 有用的替换

我们将进行一些替换以帮助说明。首先，让我们定义多项式
$$
\kappa(X) = \prod_{j=0}^{k - 1} (1 + u_{k - 1 - j} X^{2^j})
$$
这样我们可以写出$\mathbf{b}_0 = \kappa(x_3)$。系数向量$\mathbf{s}$定义为

$$\mathbf{s}_i = \prod\limits_{j=0}^{k-1} u_{k - 1 - j}^{f(i, j)}$$

其中$f(i, j)$在$i$的第$j$位被设置时返回$1$，否则返回$0$。我们也可以写出$\mathbf{G'}_0 = \innerprod{\mathbf{s}}{\mathbf{G}}$。
...
\sum_{i=0}^{k - 1} [u_j^{-1}] \rep{L_j} + \rep{P'} + \sum_{i=0}^{k - 1} [u_j] \rep{R_j} = [c] \mathbf{G'}_0 + [c z \mathbf{b}_0] U + [f] W
$$
通过检查群元素相对于$\mathbf{G}, U, W$的表示（回想一下$\alg{\prover}$是代数的，因此$\dlreladv$拥有它们），我们得到$n$个等式
$$
\sum_{i=0}^{k - 1} u_j^{-1} \repv{L_j}{G}{i} + \repv{P'}{G}{i} + \sum_{i=0}^{k - 1} u_j \repv{R_j}{G}{i} = c \mathbf{s}_i \forall i \in [0, n)
$$
以及等式
...
\end{array}
$$
的表示，这总是一个离散对数关系。如果上述任何等式不满足，则该离散对数关系是非平凡的。这是$\dlreladv$调用的函数。

### 提取器函数 $e$

提取器函数 $e$ 简单地从表示 $\rep{A_i}$ 中返回 $a_i(X)$，其中 $i \in [0, n_a)$。由于我们将对每轮中的坏挑战空间施加限制，我们保证获得的多项式使得 $g(X, C_0, C_1, \cdots, a_0(X), a_1(X), \cdots)$ 在域 $D$ 上消失，只要敌手函数 $h$ 返回的离散对数关系是平凡的。这显然表明提取器函数 $e$ 的成功概率不超过 $\pfail$，符合要求。
...
\m{U}{k} = c z \prod_{j=0}^{k - 1} (1 + u_{k - 1 - j} x_3^{2^j})
$$
我们可以通过将第一个等式的每个实例两边乘以 $\mathbf{s}_i^{-1}$（因为 $\mathbf{s}_i$ 从不为零）并在第二个等式中代入 $c$，得到以下 $n$ 个等式：
$$
\m{U}{k} = \mv{G}{i}{k} \cdot \mathbf{s}_i^{-1} z \prod_{j=0}^{k - 1} (1 + u_{k - 1 - j} x_3^{2^j}) \forall i \in [0, n)
$$
> **引理 1.** 如果 $\m{U}{k} = \mv{G}{i}{k} \cdot \mathbf{s}_i^{-1} z \prod_{j=0}^{k - 1} (1 + u_{k - 1 - j} x_3^{2^j}) \forall i \in [0, n)$，那么对于所有不包含坏挑战的记录，$\repr{P'}{U} = z \sum\limits_{i=0}^{2^k - 1} x_3^i \repv{P'}{G}{i}$。
>
> **证明。** 引入另一个抽象定义：
> $$
\z{k}{m}{i} = \mv{G}{i}{m}
$$
> 然后递归定义为所有整数 $r$ 满足 $0 \lt r \leq k$：
> $$
\z{k - r}{m}{i} = \z{k - r + 1}{m}{i} + x_3^{2^{k - r}} \z{k - r + 1}{m}{i + 2^{k - r}}
$$
> 这样我们可以将上述等式重写为：
...
$$
>
> 我们现在将证明对于所有整数 $r$ 满足 $0 \lt r \leq k$，如果以下等式对 $r$ 成立：
> $$
\m{U}{r} = \z{r}{r}{i} \cdot \mathbf{s}_i^{-1} z \prod_{j=0}^{r - 1} (1 + u_{k - 1 - j} x_3^{2^j}) \forall i \in [0, 2^r)
$$
> 那么以下等式也对 $r - 1$ 成立：
...
$$
>
> 对于所有整数 $r$ 满足 $0 \lt r \leq k$，我们有 $\mathbf{s}_{i + 2^{r - 1}} = u_{r - 1} \mathbf{s}_i \forall i \in [0, 2^{r - 1})$，根据 $\mathbf{s}$ 的定义。这给出 $\mathbf{s}_{i+2^{r - 1}}^{-1} = \mathbf{s}_i^{-1} u_{r - 1}^{-1} \forall i \in [0, 2^{r - 1})$，因为 $\mathbf{s}$ 中的任何值以及任何挑战 $u_r$ 都不为零。我们可以使用这个关系将等式的一半与另一半联系起来：
> $$
\begin{array}{rl}
\m{U}{r} &= \z{r}{r}{i} \cdot \mathbf{s}_i^{-1} z \prod_{j=0}^{r - 1} (1 + u_{k - 1 - j} x_3^{2^j}) \\
&= \z{r}{r}{i + 2^{r - 1}} \cdot \mathbf{s}_i^{-1} u_{r - 1}^{-1} z \prod_{j=0}^{r - 1} (1 + u_{k - 1 - j} x_3^{2^j}) \\
...
$$
>
> 注意 $\z{r}{r}{i}$ 可以重写为 $u_{r - 1}^{-1} \repv{L_{r - 1}}{G}{i} + \z{r}{r - 1}{i} + u_{r - 1} \repv{R_{r - 1}}{G}{i}$ 对于所有 $i \in [0, 2^{r})$。因此我们可以将上述等式重写为：
>
> $$
\begin{array}{rl}
\m{U}{r} &= \left( u_{r - 1}^{-1} \repv{L_{r - 1}}{G}{i} + \z{r}{r - 1}{i} + u_{r - 1} \repv{R_{r - 1}}{G}{i} \right) \\
...
$$
>
> 这给出了 $2^{r - 1}$ 个三元组，每个三元组是 $X$ 的最高次数为 $4$ 的多项式，尽管它们的系数在 $u_{r - 1}$ 选择之前确定，但它们仍然在 $u_{r - 1}$ 处一致。根据 Schwartz-Zippel 引理，两个多项式在 $u_{r - 1}$ 处一致但不同的概率为 $\frac{4}{|\ch|}$，因此根据并集界，这三个多项式一致但其中任何两个不同的概率为 $\frac{8}{|\ch|}$。再次根据并集界，任何 $2^{r - 1}$ 个三元组中有多个不同多项式的概率为 $\frac{2^{r - 1}\cdot8}{|\ch|}$。通过相应地限制 $u_{r - 1}$ 的挑战空间，我们得到 $|\badch(\trprefix{\tr'}{u_r})|/|\ch| \leq \frac{2^{r - 1}\cdot8}{|\ch|}$ 对于整数 $0 \lt r \leq k$，因此 $|\badch(\trprefix{\tr'}{u_k})|/|\ch| \leq \frac{4n}{|\ch|} \leq \epsilon$。
> 
> 我们现在可以得出多项式等式的结论，从而得出系数的等式。首先考虑常数项的系数，这给出了 $2^{r - 1}$ 个等式：
> $$
0 = 0 = \mathbf{s}_i^{-1} z \left( \prod_{j=0}^{r - 2} (1 + u_{k - 1 - j} x_3^{2^j}) \right) \cdot \repv{L_{r - 1}}{G}{i + 2^{r - 1}} \forall i \in [0, 2^{r - 1})
$$
>
> $\mathbf{s}$ 中的任何值都不为零，$z$ 从不选择为 $0$，并且每个 $u_j$ 选择使得 $1 + u_{k - 1 - j} x_3^{2^j}$ 不为零，因此我们可以得出结论：
> $$
0 = \repv{L_{r - 1}}{G}{i + 2^{r - 1}} \forall i \in [0, 2^{r - 1})
$$
>
> 同样的过程可以应用于等式中的 $X^4$ 项系数，以建立 $0 = \repv{R_{r - 1}}{G}{i} \forall i \in [0, 2^{r - 1})$，条件是 $x_3$ 不为零，而它总是如此。将这些代入我们的等式，我们得到更简单的形式：
> 
> $$
\begin{array}{rl}
& X \repr{L_{r - 1}}{U} + X^2 \m{U}{r - 1} + X^3 \repr{R_{r - 1}}{U} \\
...
$$
>
> 通过代入，我们得到 $\forall i \in [0, 2^{r - 1})$：
> $$
\m{U}{r - 1} = \mathbf{s}_i^{-1} z \prod_{j=0}^{r - 2} (1 + u_{k - 1 - j} x_3^{2^j}) \cdot \left( \z{r}{r - 1}{i} + \z{r}{r - 1}{i + 2^{r - 1}} x_3^{2^{r - 1}} \right)
$$
>
> 注意，根据 $\z{r - 1}{m}{i}$ 的定义，我们可以将其重写为：
>
### 提取器函数 $e$
...
>
> $$
\m{U}{r - 1} = \z{r - 1}{r - 1}{i} \cdot \mathbf{s}_i^{-1} z \prod_{j=0}^{r - 2} (1 + u_{k - 1 - j} x_3^{2^j}) \quad \forall i \in [0, 2^{r - 1})
$$
>
> 这正是我们所要证明的形式。
>
> 现在我们通过从已知成立的 $r = k$ 情况出发进行归纳，推导至 $r = 0$ 的情况，得到：
$$
\m{U}{0} = \z{0}{0}{0} \cdot \mathbf{s}_0^{-1} z
$$
>
> 由于 $\m{U}{0} = \repr{P'}{U}$ 且 $\z{0}{0}{0} = \sum_{i=0}^{2^k - 1} x_3^i \repv{P'}{G}{i}$，我们得出 $\repr{P'}{U} = z \sum_{i=0}^{2^k - 1} x_3^i \repv{P'}{G}{i}$，从而完成证明。

在确立了 $\repr{P'}{U} = z \sum_{i=0}^{2^k - 1} x_3^i \repv{P'}{G}{i}$ 之后，且给定 $x_3$ 和 $\repv{P'}{G}{i}$ 在 $z$ 的选择之前已固定，我们得出：至多存在一个非零的 $z \in \ch$ 使得 $\repr{P'}{U} = z \sum_{i=0}^{2^k - 1} x_3^i \repv{P'}{G}{i}$ 但 $\repr{P'}{U} \neq 0$。通过限制 $|\badch(\trprefix{\tr'}{z})|/|\ch| \leq \frac{1}{|\ch|} \leq \epsilon$，我们得到 $\repr{P'}{U} = 0$，因此由 $\repr{P'}{\mathbf{G}}$ 定义的多项式在 $x_3$ 处有一个根。

根据构造 $P' = P - [v] \mathbf{G}_0 + [\xi] S$，我们得出由 $\repr{P + [\xi] S}{\mathbf{G}}$ 定义的多项式在点 $x_3$ 处取值为 $v$。由于 $v, P, S$ 在 $\xi$ 的选择之前已固定，因此要么由 $\repr{S}{\mathbf{G}}$ 定义的多项式在 $x_3$ 处有一个根（这意味着由 $\repr{P}{\mathbf{G}}$ 定义的多项式在 $x_3$ 处取值为 $v$），要么 $\xi$ 是 $\ch$ 中唯一的解，使得 $\repr{P + [\xi] S}{\mathbf{G}}$ 在 $x_3$ 处取值为 $v$ 而 $\repr{P}{\mathbf{G}}$ 本身不满足。我们通过限制 $|\badch(\trprefix{\tr'}{\xi})|/|\ch| \leq \frac{1}{|\ch|} \leq \epsilon$ 来避免后一种情况，从而可以得出结论：由 $\repr{P}{\mathbf{G}}$ 定义的多项式在 $x_3$ 处取值为 $v$。

接下来的工作严格涉及证明者先前发送的群元素的表示及其与 $P$ 的关系，以及协议每一轮中选择的挑战。为了简化，我们首先用 $p(X)$ 表示由 $\repr{P}{\mathbf{G}}$ 定义的多项式，因为这里的 $p(X)$ 与协议中同名的多项式完全对应。我们将对其他群元素（及其对应的多项式）进行类似的替换，以帮助阐述，因为本证明的剩余部分主要是对Schwartz-Zippel引理的繁琐应用，以对协议中剩余每个挑战的坏挑战空间大小进行上界限制。

回想 $P = Q' + x_4 \sum\limits_{i=0}^{n_q - 1} [x_4^i] Q_i$，因此通过替换我们有 $p(X) = q'(X) + x_4 \sum\limits_{i=0}^{n_q - 1} x_4^i q_i(X)$。还回想：
...
我们已经确立 $p(x_3) = v$。注意上述 $v$ 和 $P$ 表达式中的系数在 $x_4 \in \ch$ 的选择之前已固定。根据Schwartz-Zippel引理，至多有 $n_q + 1$ 个可能的 $x_4$ 选择使得这些表达式成立，但对于某些 $i$ 有 $q_i(x_3) \neq \mathbf{u}_i$ 或：
...
通过限制 $|\badch(\trprefix{\tr'}{x_4})|/|\ch| \leq \frac{n_q + 1}{|\ch|} \leq \epsilon$，我们可以得出结论：上述所有不等式均不成立。现在我们可以将所有 $i$ 的 $\mathbf{u}_i$ 替换为 $q_i(x_3)$，得到：
...
假设 $q'(X)$（由 $\repr{Q'}{\mathbf{G}}$ 定义，且次数至多为 $n - 1$）_不_ 具有以下形式：
...
x_2^i
  ...
  \right)
$$
但 $q'(X)$ 与我们上面确立的表达式在 $x_3$ 处一致。根据Schwartz-Zippel引理，这只可能在至多 $n - 1$ 个 $x_3 \in \ch$ 的选择下发生，因此通过限制 $|\badch(\trprefix{\tr'}{x_3})|/|\ch| \leq \frac{n - 1}{|\ch|} \leq \epsilon$，我们得到：
...
接下来，我们将通过再次应用Schwartz-Zippel引理，在 $x_2$ 上提取该多项式的系数（这些系数本身是形式变量 $X$ 的多项式）；这同样导致限制 $|\badch(\trprefix{\tr'}{x_2})|/|\ch| \leq \frac{n_q}{|\ch|} \leq \epsilon$，并且对于所有 $i \in [0, n_q - 1)$，我们得到以下次数至多为 $n - 1$ 的多项式：
...
在确立了这些均为次数至多为 $n - 1$ 的非有理多项式后，我们可以说（根据因子定理）对于每个 $i \in [0, n_q - 1]$ 和 $j \in [0, n_e - 1]$，$q_i(X) - r_i(X)$ 在 $\omega^{\left(\mathbf{q_i}\right)_j} x$ 处有一个根。注意，我们可以将每个 $q_i(X)$ 解释为在点 $x_1$ 处对某个_双变量_多项式的限制，该多项式在 $x_1$ 上的次数至多为 $n_a + 1$，其系数由各种多项式 $a'_i(X)$（来自表示 $\repr{A'_i}{\mathbf{G}}$）、$h'(X)$（来自表示 $\repr{H'_i}{\mathbf{G}}$）和 $r(X)$（来自表示 $\repr{R}{\mathbf{G}}$）组成。通过类似地应用Schwartz-Zippel引理并限制挑战空间 $|\badch(\trprefix{\tr'}{x_1})|/|\ch| \leq \frac{n_a + 1}{|\ch|} \leq \epsilon$，我们得到（根据协议步骤12和13中每个 $q'_i(X)$ 和 $r_i(X)$ 的构造）：证明者在步骤9中声称的 $r$ 值等于 $r(x)$；验证者在步骤13中计算的 $h$ 值等于 $h'(x)$；并且对于所有 $i \in [0, n_q - 1]$，证明者声称的值 $(\mathbf{a_i})_j = a'_i(\omega^{(\mathbf{p_i})_j} x)$ 对所有 $j \in [0, n_e - 1]$ 成立。

根据步骤7中 $h'(X)$（来自表示 $\repr{H'}{\mathbf{G}}$）的构造，我们知道 $h'(x) = h(x)$，其中 $h(X)$ 是次数至多为 $(n_g - 1) \cdot (n - 1)$ 的多项式，其系数对应于每个 $\repr{H_i}{\mathbf{G}}$ 的串联表示。与之前一样，假设 $h(X)$ _不_ 具有 $g'(X) / t(X)$ 的形式。由于 $h(X)$ 在 $x$ 的选择之前已确定，根据Schwartz-Zippel引理，我们知道如果多项式不相等，它最多只能在 $(n_g - 1) \cdot (n - 1)$ 个点上与 $g'(X) / t(X)$ 一致。通过再次限制 $|\badch(\trprefix{\tr'}{x})|/|\ch| \leq \frac{(n_g - 1) \cdot (n - 1)}{|\ch|} \leq \epsilon$，我们得到 $h(X) = g'(X) / t(X)$，并且由于 $h(X)$ 是一个非有理多项式，根据因子定理，我们得出 $g'(X)$ 在域 $D$ 上消失。

我们现在有 $g'(X)$ 在 $D$ 上消失，但希望证明 $g(X, C_0, C_1, \cdots)$ 在 $D$ 的所有点上消失以完成证明。这仅涉及对每个挑战应用相同技术的一系列操作；由于根据定义，多项式 $g(\cdots)$ 在任何不定元上的次数至多为 $n_g \cdot (n - 1)$，并且由于每个多项式 $a_i(X, C_0, C_1, ..., C_{i - 1}, \cdots)$ 在具体挑战 $c_i$ 的选择之前已确定，通过类似地限制 $|\badch(\trprefix{\tr'}{c_i})|/|\ch| \leq \frac{n_g \cdot (n - 1)}{|\ch|} \leq \epsilon$，我们确保 $g(X, C_0, C_1, \cdots)$ 在 $D$ 上消失，从而完成证明。